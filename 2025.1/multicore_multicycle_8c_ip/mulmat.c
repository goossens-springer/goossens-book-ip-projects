#define LOG_NB_IP                       3  //1, 2 or 3
#define NB_IP               (1<<LOG_NB_IP) //2, 4 or 8
#define LINE_X                         64
#define COLUMN_X                       96
...
#define LINE_BX             (LINE_X/NB_IP)
#define COLUMN_BX                COLUMN_X
#define LINE_BY                    LINE_Y
#define COLUMN_BY         (COLUMN_Y/NB_IP)
#define LINE_BZ                   LINE_BX
#define COLUMN_BZ               COLUMN_BY
#define LOG_DATA_RAM_SIZE              15
#define DATA_RAM_SIZE (1<<LOG_DATA_RAM_SIZE)
#define LOG_IP_DATA_RAM_SIZE  (15-LOG_NB_IP)
#define IP_DATA_RAM_SIZE (1<<LOG_IP_DATA_RAM_SIZE) //2^(16-LOG_NB_IP) words
#define SIZE_X   (LINE_X*COLUMN_X/NB_IP)
#define SIZE_XY  (LINE_Y*COLUMN_Y/NB_IP + SIZE_X)
...
void main(){
 ...
 for (int k=0; k<NB_IP; k++){
  offset_block_y=k*COLUMN_BY;
  for (int c=0; c<NB_IP; c++){
    read_block(((int*)block_y)+c*LINE_BY/NB_IP*COLUMN_BY,LINE_BY/NB_IP,COLUMN_BY,c*IP_DATA_RAM_SIZE+SIZE_X+offset_block_y,COLUMN_Y);
  }
