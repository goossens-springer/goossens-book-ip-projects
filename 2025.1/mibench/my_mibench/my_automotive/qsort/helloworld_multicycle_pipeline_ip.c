...
int main(){
  ...
  for (i=0; i<6*ARRAYSIZE; i+=6){
    XMulticycle_pipeline_ip_Read_data_ram_Words(&ip, i,   &n0, 1);
    XMulticycle_pipeline_ip_Read_data_ram_Words(&ip, i+1, &n1, 1);
    XMulticycle_pipeline_ip_Read_data_ram_Words(&ip, i+2, &n2, 1);
