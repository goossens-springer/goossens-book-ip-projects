...
int main(){
  ...
  printf("Solutions:");
  XMulticycle_pipeline_ip_Read_data_ram_Words(&ip, gi, (word_type*)&solutions, 1);
  gi++;
  for(i=0;i<solutions;i++){
    XMulticycle_pipeline_ip_Read_data_ram_Words(&ip, gi, (word_type*)&d, 2);
    gi+=2;
    printf(" %f",d);
  }
  printf("\n");
  ...
  //for (i = 0; i < 1001; ++i)
    {
     XMulticycle_pipeline_ip_Read_data_ram_Words(&ip, gi, (word_type*)&u, 2);
     gi+=2;
     printf("sqrt(%3d) = %2ld\n", i, u);
    }
  XMulticycle_pipeline_ip_Read_data_ram_Words(&ip, gi, (word_type*)&u, 2);
