...
typedef struct {
    ...
    XRv32i_npp_ip_uio_map maps[ MAX_UIO_MAPS ];
} XRv32i_npp_ip_uio_info;

/***************** Variable Definitions **************************************/
static XRv32i_npp_ip_uio_info uio_info;
...
static int uio_info_read_name(XRv32i_npp_ip_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XRv32i_npp_ip_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XRv32i_npp_ip_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XRv32i_npp_ip_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XRv32i_npp_ip_Initialize(XRv32i_npp_ip *InstancePtr, const char* InstanceName) {
	XRv32i_npp_ip_uio_info *InfoPtr = &uio_info;
