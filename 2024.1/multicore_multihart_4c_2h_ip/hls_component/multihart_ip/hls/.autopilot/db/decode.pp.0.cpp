...
void init_d_state( bit_t *d_state_is_full){
  hart_num_p1_t h1;
  hart_num_t h;
  VITIS_LOOP_14_1: for (h1=0; h1<(1<<1); h1++){
#pragma HLS UNROLL
 h = h1;
    d_state_is_full[h] = 0;
  }
...
static void decode_instruction(
  ...
  is_lui = (opcode == 0b01101);
  is_branch = (opcode == 0b11000);
  is_jal = (opcode == 0b11011);
  is_jalr = (opcode == 0b11001);
  is_load = (opcode == 0b00000);
  is_store = (opcode == 0b01000);
  is_op_imm = (opcode == 0b00100);
  is_not_auipc = (opcode != 0b00101);
  is_not_jal = !is_jal;
  d_i->opcode = opcode;
...
static void stage_job(
  ...
  d_state[hart].relative_pc =
    (d_i.is_jal)?
      (ip_code_address_t)(d_state[hart].fetch_pc +
      (ip_code_address_t)(d_i.imm >> 1)):
      (ip_code_address_t)(d_state[hart].fetch_pc + 1);
}
