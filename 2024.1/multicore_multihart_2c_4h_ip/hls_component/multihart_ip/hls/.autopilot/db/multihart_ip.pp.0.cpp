...
typedef ap_uint<(16 -1)> ip_code_address_t;
...
typedef ap_uint<(16 -1)> w_ip_data_address_t;
typedef ap_uint<(16 -1)+1> h_ip_data_address_t;
typedef ap_uint<(16 -1)+2> b_ip_data_address_t;
typedef ap_uint<((16 -1)-2)> w_hart_data_address_t;
typedef ap_uint<((16 -1)-2)+1> h_hart_data_address_t;
typedef ap_uint<((16 -1)-2)+2> b_hart_data_address_t;
...
typedef ap_uint<1> ip_num_t;
typedef ap_uint<2 +1> hart_num_p1_t;
typedef ap_uint<2> hart_num_t;
typedef ap_uint<(1<<2)> hart_set_t;
...
__attribute__((sdx_kernel("multihart_ip", 0))) void multihart_ip(
  ...
  unsigned int start_pc [(1<<2)],
  unsigned int ip_code_ram[(1<<(16 -1))],
  int ip_data_ram[(1<<2)][(1<<((16 -1)-2))],
  int data_ram [(1<<1)][(1<<2)][(1<<((16 -1)-2))],
...
void mem_access(
  ...
  int ip_data_ram[][(1<<((16 -1)-2))],
  int data_ram[][(1<<2)][(1<<((16 -1)-2))],
  m_state_t *m_state,
  from_m_to_w_t *m_to_w,
  bit_t *m_state_is_full);
...
static void init_file(
  ...
  reg_num_p1_t r1;
  reg_num_t r;
  VITIS_LOOP_28_1: for (h1=0; h1<(1<<2); h1++){
#pragma HLS UNROLL
 h = h1;
...
#pragma HLS UNROLL
 r = r1;
      ...
      if (r==10)
        ...
        reg_file [h][r] = ((int)(ip_num+1))<<((16 -1)+2);
      else
        reg_file [h][r] = 0;
    }
  }
...
static void init_exit(
  ...
  hart_num_t h;
  bit_t h_running;
  VITIS_LOOP_52_1: for (h1=0; h1<(1<<2); h1++){
#pragma HLS UNROLL
 h = h1;
...
static void running_cond_update(
  ...
  bit_t cond;
  cond = 0;
  VITIS_LOOP_66_1: for (h1=0; h1<(1<<2); h1++){
#pragma HLS UNROLL
 h = h1;
