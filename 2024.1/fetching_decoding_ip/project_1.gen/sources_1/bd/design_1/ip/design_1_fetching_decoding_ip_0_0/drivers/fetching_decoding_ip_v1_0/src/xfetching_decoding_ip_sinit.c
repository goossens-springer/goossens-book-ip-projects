...
#ifdef SDT
XFetching_decoding_ip_Config *XFetching_decoding_ip_LookupConfig(UINTPTR BaseAddress) {
	XFetching_decoding_ip_Config *ConfigPtr = NULL;

	int Index;

	for (Index = (u32)0x0; XFetching_decoding_ip_ConfigTable[Index].Name != NULL; Index++) {
		if (!BaseAddress || XFetching_decoding_ip_ConfigTable[Index].Control_BaseAddress == BaseAddress) {
...
int XFetching_decoding_ip_Initialize(XFetching_decoding_ip *InstancePtr, UINTPTR BaseAddress) {
	XFetching_decoding_ip_Config *ConfigPtr;

	Xil_AssertNonvoid(InstancePtr != NULL);

	ConfigPtr = XFetching_decoding_ip_LookupConfig(BaseAddress);
	if (ConfigPtr == NULL) {
