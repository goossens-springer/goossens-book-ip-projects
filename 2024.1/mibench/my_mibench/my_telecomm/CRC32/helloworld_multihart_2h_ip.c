...
int main(){
  ...
  for (int h=0; h<NB_HART; h++){
    printf("hart %d\n", h);
    XMultihart_ip_Read_data_ram_Words(&ip, (h<<LOG_HART_DATA_RAM_SIZE) + CRC/4, &crc, 1);
    XMultihart_ip_Read_data_ram_Words(&ip, (h<<LOG_HART_DATA_RAM_SIZE) + CHARCNT/4, &charcnt, 1);
