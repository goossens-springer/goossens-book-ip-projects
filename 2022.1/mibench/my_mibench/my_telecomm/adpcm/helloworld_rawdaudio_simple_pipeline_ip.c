...
int main(){
  ...
  XSimple_pipeline_ip_Read_data_ram_Bytes(&ip, STATE+0, &valprev0, 1);
  XSimple_pipeline_ip_Read_data_ram_Bytes(&ip, STATE+1, &valprev1, 1);
  valprev = (((unsigned short)valprev1)<<8)|((unsigned short)valprev0);
  XSimple_pipeline_ip_Read_data_ram_Bytes(&ip, STATE+2, &index,  1);
  XSimple_pipeline_ip_Read_data_ram_Words(&ip, LENGTH/4, &length, 1);
  printf("Final valprev=%d, index=%d\n", valprev, index);
  for (i=0; i<length/20; i++){
    for (j=0; j<20; j++){
      XSimple_pipeline_ip_Read_data_ram_Bytes(&ip, OUTPUT+i*20+j, &c, 1);
      printf("%3d ", c);
    }
    printf("\n");
  }
