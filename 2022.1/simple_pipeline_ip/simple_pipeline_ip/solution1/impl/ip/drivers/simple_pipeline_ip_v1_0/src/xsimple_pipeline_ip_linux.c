...
typedef struct {
    ...
    XSimple_pipeline_ip_uio_map maps[ MAX_UIO_MAPS ];
} XSimple_pipeline_ip_uio_info;

/***************** Variable Definitions **************************************/
static XSimple_pipeline_ip_uio_info uio_info;
...
static int uio_info_read_name(XSimple_pipeline_ip_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XSimple_pipeline_ip_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XSimple_pipeline_ip_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XSimple_pipeline_ip_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XSimple_pipeline_ip_Initialize(XSimple_pipeline_ip *InstancePtr, const char* InstanceName) {
	XSimple_pipeline_ip_uio_info *InfoPtr = &uio_info;
